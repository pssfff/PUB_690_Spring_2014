...
Sangsoo Park
========================================================
Derive $\beta_0$ and $\beta_1$

$$Q = \sum_{i=1}^{n}(y_i - (\beta_0 + \beta_1*x_i))^2$$
$$= \sum_{i=1}^{n}(y_i^2 - 2*y_i*(\beta_0 + \beta_1*x_i) + (\beta_0 + \beta_1*x_i)^2)$$

To minimize Q, partial derivative of Q by either $\beta_0$ or $\beta_1$ must be zero

1) With regard to $\beta_0$

$$\frac{\partial Q}{\partial \beta_0} = \sum_{i=1}^{n}(-2*y_i + 2*\beta_0 + 2*\beta_1*x_i)$$
...
2) With regard to $\beta_1$

$$\frac{\partial Q}{\partial \beta_1} = \sum_{i=1}^{n}(-2*x_i*y_i + 2*\beta_0*x_i + 2*\beta_1*x_i^2) = 0$$
$$\sum_{i=1}^{n}(\beta_1*x_i^2) = \sum_{i=1}^{n}(x_i*y_i) - \sum_{i=1}^{n}(\beta_0*x_i)$$ 
$$= \sum_{i=1}^{n}(x_i*y_i) - (\mu(y)-\beta_1*\mu(x))*\sum_{i=1}^{n}(x_i)$$ 
$$= \sum_{i=1}^{n}(x_i*y_i) - \mu(y) * \sum_{i=1}^{n}(x_i) + \beta_i*\mu(x) * \sum_{i=1}^{n}(x_i)$$
$$\beta_1*(\sum_{i=1}^{n}(x_i^2) - \mu(x) * \sum_{i=1}^{n}(x_i))$$
$$= \sum_{i=1}^{n}(x_i*(y_i - \mu(y)))$$
$$\beta_1 = \frac{\sum_{i=1}^{n}(x_i*(y_i-\mu(y)))}{\sum_{i=1}^{n}(x_i*(x_i-\mu(x)))}$$
